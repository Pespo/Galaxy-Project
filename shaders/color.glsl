...
{	
	uv = vertexUv;
	normal = vec3(model * vec4(vertexNormal, 0.0));
...
vec3 spotLight(in vec3 lcolor, in float intensity, in vec3 ldir, in vec3 lpos, in vec3 n, in vec3 fpos, vec3 diffuse, float spec, vec3 cpos)
{
	...
	if (cosTs > cosTp) 
		color1 = pow(cosTs, 30.0) * lcolor * intensity * (diffuse * n_dot_l + spec * vec3(1.0, 1.0, 1.0) *  pow(n_dot_h, spec * 100.0));
	return color1;
}
...
void main(void)
{
	...
	vec3 cpointlight1 = pointLight(vec3(1.0, 1.0, 1.0), 20.0, vec3(0.0, 0.0, 0.0), n, position, color.rgb, spec, cameraPosition);
	//vec3 cpointlight2 = pointLight(vec3(1.0, 0.0, 0.0), 1.0, vec3(10.0, 1.0, 10.0), n, position, diffuse, spec, cameraPosition);
	//vec3 cdirlight1 = directionalLight(vec3(1.0, 0.0, 0.0), 10.0, vec3(0.0, -1.0, 0.0), n, position, color.rgb, spec, cameraPosition);
