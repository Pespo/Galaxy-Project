...
private:
    struct ObjectInstance {
        ...
        ObjectInstance(size_t id, size_t objectId, size_t shaderId, GLuint textureId0, GLuint textureId1, const Matrix4f& transformation, const Color&color, GLuint materialId, GLuint lightId) :
            id(id), objectId(objectId), shaderId(shaderId), textureId0(textureId0), textureId1(textureId1), transformation(transformation), color(color), materialId(materialId), lightId(lightId) {
        }
    };
    ...
    GLuint defaultLightID; // Default light for drawn elements
...
public:
    ...
    void setDrawnObjectLightID(GLuint indexDrawnObject, GLuint lightID);
    ...
    void setDefaultLightID(GLuint defaultLightID);
